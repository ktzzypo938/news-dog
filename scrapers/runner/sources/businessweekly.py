...
CATEGORIES = {
    'politics': {
        'label': '政治',
        'urls': (
            'https://www.businessweekly.com.tw/channel/news/0000000313',
            'https://www.businessweekly.com.tw/channel/focus/0000000312',
        ),
...
ARTICLE_RE = re.compile(r'^https://www\.businessweekly\.com\.tw/[a-z-]+/(?:blog|article)/\d+$')
URL_CATEGORY_MAP = {}
...
def _extract_entries(soup):
    entries = []
    for item in soup.select('figure.Article-figure'):
...
def _clean_title(title):
    return re.sub(r'[－|-].*商周$', '', str(title)).strip()


def _extract_clean_text(soup):
    content_node = (
        soup.select_one('.Single-article.WebContent')
        or soup.select_one('.Single-article')
    ...
    for tag in content.select(
        'script, style, iframe, img, figure, aside, .Ad, .Tag, .Collect-btn, '
        '.Article-tool, .Related, .Keyword, .Single-page'
    ):
        tag.decompose()
...
def _extract_photographer(soup):
    for node in soup.select('figcaption, .Single-img-caption, img[alt]'):
